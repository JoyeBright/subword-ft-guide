...
MODELS="$ROOT/Models"
LOG_DIR="$ROOT/log/evaluation_logs_wmt"
EVAL_CONFIGS="$ROOT/eval_configs_wmt"
OUTPUT_CSV="$ROOT/evaluation_scores_wmt.csv"
EVAL_LOG_DIR="$LOG_DIR"
PRED_ROOT="$ROOT/Models_wmt_pred"
mkdir -p "$LOG_DIR" "$EVAL_CONFIGS" "$PRED_ROOT"
...
# WMT medical test set for all configs
TEST_SRC="/home/pourmost/Redo/RANLP/data/wmt/test.src.tok.bpe_wmt"
TEST_TGT="/home/pourmost/Redo/RANLP/data/wmt/test.tgt.tok.bpe_wmt"

declare -a CONFIGS=(
"C0|wmt_base|wmt_base|wmt_base|wmt_base baseline"
...
for CONFIG in "${CONFIGS[@]}"; do
  ...
  # Special case for wmt_base model
  if [[ "$ID" == "C0" ]]; then
    MODEL_PATH="$MODELS/wmt_base/model_step_48000.pt"
  else
  ...
  # Skip evaluation if prediction already exists and is not empty
  if [[ -s "$PRED_FILE" ]]; then
    echo "⚡ Skipping $ID ($NAME) -- prediction already exists: $PRED_FILE"
    continue
  fi
  ...
  # Create YAML config for OpenNMT
  cat <<EOF > "$YAML_PATH"
model: $MODEL_PATH
src: $SRC_FILE
output: $PRED_FILE
...
EOF
  ...
  echo "$ID,$NAME,$BPE_MODEL,$VOC_SRC,\"$FT_DESC\",wmt_bpe_medical,$BLEU,$COMET,$CO2,$TIME" >> "$OUTPUT_CSV"
done

echo "✅ All WMT-medical evaluations complete. Results saved to $OUTPUT_CSV"
